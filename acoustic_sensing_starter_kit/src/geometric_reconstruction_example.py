...
def geometric_reconstruction_example():
    ...
    scenarios = {
        ...
        'edge_contact': np.sin(2 * np.pi * 1500 * t) + 0.5 * np.sin(2 * np.pi * 3000 * t) + 0.1 * np.random.randn(len(t))
    }
    ...
    for scenario in scenarios.keys():
        ...
        for mode in modes:
            ...
            print(f"  {mode:8s}: {len(features)} features, range: [{features.min():.2e}, {features.max():.2e}]")
...
def workflow_integration_example():
    ...
    class GeometricReconstructionSystem:
        ...
        def process_contact(self, audio_signal, contact_type_hint=None):
            ...
            return {
                ...
                'computation_time_estimate': self.extractor.config.config['computation_time']
            }
    ...
    for priority in priorities:
        ...
        print(f"   Mode: {result['mode']}, Features: {result['n_features']}, "
