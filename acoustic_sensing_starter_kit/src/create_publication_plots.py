...
class ComprehensiveSummaryVisualizer:
    ...
    def load_all_results(self) -> Dict:
        ...
        batch_names = ['soft_finger_batch_1', 'soft_finger_batch_2', 
                      'soft_finger_batch_3', 'soft_finger_batch_4']
    ...
    def create_feature_importance_heatmap(self, all_results: Dict):
        ...
        research_features = optimal_features + [
            'mid_energy_ratio', 'resonance_peak_amp', 'env_max'
        ]
            ...
            if 'saliency' in results:
                ...
                for feature in research_features:
                    if feature in saliency_df['feature_name'].values:
                        importance = saliency_df[saliency_df['feature_name'] == feature]['rf_builtin'].iloc[0]
        ...
        for i, feature in enumerate(research_features):
            if feature in optimal_features:
                rect = plt.Rectangle((i, 0), 1, len(batch_labels), 
                                   fill=False, edgecolor='blue', lw=2)
                ax.add_patch(rect)
        
        ax.set_title('Feature Importance Across Experimental Batches\n' + 
                    '(Blue boxes indicate OPTIMAL feature set)', fontsize=14, fontweight='bold')
        ax.set_xlabel('Acoustic Features', fontsize=12)
        ax.set_ylabel('Experimental Batches', fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        
        plt.savefig(self.output_dir / 'feature_importance_heatmap.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_ablation_summary_plot(self, all_results: Dict):
        ...
        fig.suptitle('Ablation Analysis Summary: Validating Feature Importance', 
        ...
        sorted_features = sorted(feature_appearances.items(), key=lambda x: x[1], reverse=True)
        ...
        bars1 = ax1.barh(range(len(feature_names)), appearances, color=colors, alpha=0.7)
        ...
        for batch_name, perfs in performance_data.items():
            ax2.plot(feature_counts, perfs, 'o-', label=batch_name, linewidth=2, markersize=8)
        
        ax2.axvline(x=5, color='red', linestyle='--', alpha=0.7, label='OPTIMAL (5 features)')
        ...
        optimal_scores = [s for s, l in zip(all_importance_scores, all_feature_labels) if l == 'OPTIMAL']
        other_scores = [s for s, l in zip(all_importance_scores, all_feature_labels) if l == 'Other']
        
        ax3.hist([optimal_scores, other_scores], bins=20, alpha=0.7, 
                label=['OPTIMAL features', 'Other features'], color=['blue', 'gray'])
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'ablation_analysis_summary.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_performance_comparison_plot(self):
        ...
        fig.suptitle('Feature Set Performance Comparison: Validation Results', 
                    fontsize=16, fontweight='bold')
        ...
        colors = [self.colors['minimal'], self.colors['optimal'], self.colors['research']]
        bars1 = ax1.bar(modes, accuracies, color=colors, alpha=0.8, edgecolor='black', linewidth=1)
        ...
        # Accuracy line
        line1 = ax2.plot(range(len(modes)), accuracies, 'o-', color='blue', 
                        linewidth=3, markersize=10, label='Accuracy (%)')
        ax2.set_ylabel('Accuracy (%)', color='blue')
        ax2.set_ylim([94, 100])
        
        # Computation time bars
        bars2 = ax2_twin.bar(range(len(modes)), computation_times, alpha=0.5, 
                            color='orange', width=0.5, label='Computation (ms)')
        ...
        efficiency_scores = [acc/time for acc, time in zip(accuracies, computation_times)]
        best_idx = efficiency_scores.index(max(efficiency_scores))
        
        ax2.annotate(f'Best Efficiency\n{efficiency_scores[best_idx]:.1f} acc/ms', 
    ...
    def create_master_summary_plot(self, all_results: Dict):
        ...
        # Title
        fig.suptitle('Acoustic Feature Selection for Geometric Reconstruction: Comprehensive Analysis', 
        ...
        feature_summary = []
        optimal_features = ['spectral_bandwidth', 'spectral_centroid', 'high_energy_ratio',
                           'ultra_high_energy_ratio', 'temporal_centroid']
        ...
        bars1 = ax1.barh(range(len(feature_names)), mean_scores, color=colors, alpha=0.8)
        ax1.set_yticks(range(len(feature_names)))
        ax1.set_yticklabels(feature_names, fontsize=9)
        ax1.set_xlabel('Mean Feature Importance')
        ax1.set_title('Top 15 Features Ranked by Importance\n(Red = OPTIMAL set)', fontweight='bold')
        ax1.grid(True, alpha=0.3)
        ...
        bars1 = ax2.bar(x - width, minimal_acc, width, label='MINIMAL (2)', color=self.colors['minimal'], alpha=0.8)
        bars2 = ax2.bar(x, optimal_acc, width, label='OPTIMAL (5)', color=self.colors['optimal'], alpha=0.8)
        bars3 = ax2.bar(x + width, research_acc, width, label='RESEARCH (8)', color=self.colors['research'], alpha=0.8)
        ...
        box_plot = ax3.boxplot(data_to_plot, labels=['OPTIMAL\nFeatures', 'Other\nFeatures'], 
        ...
        scatter = ax4.scatter(computation_times[:-1], avg_accuracies[:-1], 
                             s=[200, 400, 300], c=[self.colors['minimal'], self.colors['optimal'], self.colors['research']], 
                             alpha=0.8, edgecolors='black', linewidth=2)
        
        # Add original point for reference
        ax4.scatter(computation_times[-1], avg_accuracies[-1], s=100, c='red', 
                   marker='x', linewidth=3, label='Original (38 features)')
        
        # Connect points to show frontier
        ax4.plot(computation_times[:-1], avg_accuracies[:-1], '--', alpha=0.5, color='gray')
        ...
        for i, (x, y, label) in enumerate(zip(computation_times[:-1], avg_accuracies[:-1], labels)):
            ax4.annotate(f'{label}\n({feature_counts[i]} feat)', (x, y), 
...
"""
        ...
        plt.savefig(self.output_dir / 'master_summary_publication.png', dpi=300, bbox_inches='tight')
        plt.close()
