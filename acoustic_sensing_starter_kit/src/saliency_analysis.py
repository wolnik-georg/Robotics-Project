...
class AdvancedAcousticCNN(nn.Module):
    ...
    def __init__(self, input_size: int, n_classes: int, architecture: str = 'spectro_temporal'):
        super().__init__()
...
class AcousticSaliencyAnalyzer:
    ...
    def load_batch_data(self, batch_name: str, data_type: str = 'features') -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def train_model(self, batch_name: str, data_type: str = 'features', 
                   architecture: str = None, epochs: int = 100) -> float:
        """
    ...
    def compute_gradient_saliency(self, batch_name: str, data_type: str = 'features', 
    ...
    def compute_integrated_gradients(self, batch_name: str, data_type: str = 'features',
                                   sample_idx: int = 0, steps: int = 50) -> np.ndarray:
        """
    ...
    def analyze_batch_saliency(self, batch_name: str, num_samples: int = 10) -> Dict:
        ...
        data_types = ['features']  # Start with features, can extend to raw_audio, spectrograms
        ...
        for data_type in data_types:
            try:
                ...
                for i in range(min(num_samples, len(self.models[f"{batch_name}_{data_type}"]['test_data'][0]))):
                    # Gradient-based saliency
                    grad_sal = self.compute_gradient_saliency(batch_name, data_type, i)
                    gradient_saliencies.append(grad_sal)
                    
                    # Integrated gradients
                    int_grad_sal = self.compute_integrated_gradients(batch_name, data_type, i)
                ...
                results['saliency_maps'][data_type] = {
                    ...
                    'integrated_gradients_std': np.std(integrated_grad_saliencies, axis=0)
                }
                ...
                if data_type == 'features' and LIME_AVAILABLE:
                    try:
                        ...
                        results['feature_importance']['lime'] = lime_result['feature_importance']
    ...
    def visualize_saliency_maps(self, batch_name: str, save_dir: Path = None):
        ...
        fig.suptitle(f'Saliency Analysis: {batch_name}\n{results["config"]["description"]}', 
        ...
        if 'features' in results['saliency_maps']:
            ...
            try:
                ...
                if features_file.exists():
                    ...
                    feature_names = [f'Feature_{i}' for i in range(len(results['saliency_maps']['features']['gradient']))]
            except:
                feature_names = [f'Feature_{i}' for i in range(len(results['saliency_maps']['features']['gradient']))]
            ...
            bars2 = ax2.bar(range(len(int_grad_sal)), int_grad_sal, color='lightgreen', alpha=0.7)
            ...
            width = 0.35
            ax3.bar(x_pos - width/2, comparison_data['Gradient'], width, 
                   label='Gradient', alpha=0.7, color='skyblue')
            ax3.bar(x_pos + width/2, comparison_data['Integrated Gradients'], width,
                   label='Integrated Gradients', alpha=0.7, color='lightgreen')
        ...
        if save_dir:
            ...
            plt.savefig(save_dir / f"{batch_name}_saliency_analysis.png", dpi=300, bbox_inches='tight')
    ...
    def _save_detailed_results(self, batch_name: str, save_dir: Path):
        ...
        if 'features' in results['saliency_maps']:
            saliency_df = pd.DataFrame({
                'feature_index': range(len(results['saliency_maps']['features']['gradient'])),
                'gradient_saliency': results['saliency_maps']['features']['gradient'],
                'integrated_gradients': results['saliency_maps']['features']['integrated_gradients'],
                'gradient_std': results['saliency_maps']['features']['gradient_std'],
                'integrated_gradients_std': results['saliency_maps']['features']['integrated_gradients_std']
            })
            ...
            try:
                ...
                saliency_df['feature_name'] = [f'feature_{i}' for i in range(len(saliency_df))]
            
            saliency_df.to_csv(save_dir / f"{batch_name}_saliency_maps.csv", index=False)
        ...
        if 'features' in results['saliency_maps']:
            ...
            summary['top_features'] = {
                ...
                'integrated_gradients_values_top_10': int_grad_sal[np.argsort(int_grad_sal)[-10:]].tolist()
            }
        ...
        with open(summary_path, "w") as f:
            ...
            for batch_name, result in all_results.items():
                ...
                if 'features' in result['saliency_maps']:
                    ...
                    int_grad_sal = result['saliency_maps']['features']['integrated_gradients']
                    
                    f.write(f"Top Gradient Features: {np.argsort(grad_sal)[-5:].tolist()}\n")
                    f.write(f"Top Int. Grad Features: {np.argsort(int_grad_sal)[-5:].tolist()}\n")
                    f.write(f"Saliency Consistency (corr): {np.corrcoef(grad_sal, int_grad_sal)[0,1]:.3f}\n")
