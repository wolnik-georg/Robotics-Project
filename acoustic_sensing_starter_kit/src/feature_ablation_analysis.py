...
class FeatureAblationAnalyzer:
    ...
    def __init__(self, batch_configs: Dict, base_data_dir: str):
        ...
        self.feature_groups = {
            'spectral': ['spectral_centroid', 'spectral_bandwidth', 'spectral_rolloff', 
                        'spectral_flatness', 'spectral_contrast_mean'],
            'energy_ratios': ['ultra_high_energy_ratio', 'high_energy_ratio', 
                            'mid_energy_ratio', 'low_energy_ratio', 'ultra_high_ratio', 
                            'high_ratio', 'mid_ratio', 'low_ratio'],
            'temporal': ['temporal_centroid', 'env_mean', 'env_std', 'env_skew', 
                        'env_kurtosis', 'attack_time', 'decay_time'],
    ...
    def load_batch_data(self, batch_name: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    ...
    def get_baseline_performance(self, X: np.ndarray, y: np.ndarray, 
        ...
        print(f"  Baseline RF accuracy: {baseline_results['RandomForest']['mean_accuracy']:.3f} ± {baseline_results['RandomForest']['std_accuracy']:.3f}")
        
        return baseline_results
        
    def leave_one_out_ablation(self, X: np.ndarray, y: np.ndarray, 
                              feature_names: List[str]) -> Dict:
    ...
    def top_features_ablation(self, X: np.ndarray, y: np.ndarray, 
        ...
        for n_remove in range(1, min(n_top + 1, len(top_feature_indices) + 1)):
            ...
            print(f"  Removed top {n_remove}: {cv_scores.mean():.3f} ± {cv_scores.std():.3f}")
            
        return top_ablation_results
        
    def feature_group_ablation(self, X: np.ndarray, y: np.ndarray, 
                              feature_names: List[str]) -> Dict:
        ...
        for group_name, group_features in self.feature_groups.items():
            ...
            print(f"  Removed {group_name} ({len(group_indices)} features): {cv_scores.mean():.3f} ± {cv_scores.std():.3f}")
            
        return group_results
        
    def cumulative_feature_addition(self, X: np.ndarray, y: np.ndarray, 
                                   feature_names: List[str], 
                                   feature_ranking: List[int]) -> Dict:
        ...
        for n_features in range(1, min(21, len(feature_ranking) + 1)):  # Test up to 20 features
            # Use top n features
            ...
            if n_features <= 10 or n_features % 5 == 0:
                print(f"  Top {n_features} features: {cv_scores.mean():.3f} ± {cv_scores.std():.3f}")
                
        return cumulative_results
        
    def random_feature_combinations(self, X: np.ndarray, y: np.ndarray, 
                                   feature_names: List[str], 
        ...
        for n_features in [5, 10, 15, 20]:
            ...
            for trial in range(n_trials):
                ...
                # Cross-validation
                cv_scores = cross_val_score(clf, X_random, y, cv=3, scoring='accuracy')  # Faster CV
            ...
            print(f"  Random {n_features} features: {np.mean(trial_results):.3f} ± {np.std(trial_results):.3f} (range: {np.min(trial_results):.3f}-{np.max(trial_results):.3f})")
            
        return random_results
        
    def analyze_batch_ablation(self, batch_name: str, 
        ...
        importance_file = self.results_dir / batch_name / f"{batch_name}_feature_saliency.csv"
        ...
        batch_results['leave_one_out'] = self.leave_one_out_ablation(X, y, feature_names)
        ...
        for feature_name in batch_results['leave_one_out']:
            acc_without_feature = batch_results['leave_one_out'][feature_name]['mean_accuracy']
            batch_results['leave_one_out'][feature_name]['accuracy_drop'] = baseline_acc - acc_without_feature
        ...
        batch_results['group_ablation'] = self.feature_group_ablation(X, y, feature_names)
    ...
    def save_ablation_report(self, batch_name: str, results: Dict, batch_dir: Path):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"Random Forest Accuracy: {baseline['mean_accuracy']:.4f} ± {baseline['std_accuracy']:.4f}\n\n")
            
            # Most important features (by leave-one-out)
            loo = results['leave_one_out']
            sorted_features = sorted(loo.items(), key=lambda x: x[1]['accuracy_drop'], reverse=True)
            ...
            for i, (feature_name, data) in enumerate(sorted_features[:10]):
                f.write(f"{i+1:2d}. {feature_name:25s} | Drop: {data['accuracy_drop']:.4f} | Acc without: {data['mean_accuracy']:.4f}\n")
            ...
            for target_acc in [0.95, 0.90, 0.85]:
                ...
                for key, data in cum_results.items():
                    if data['mean_accuracy'] >= target_acc * baseline_acc:
                        f.write(f"≥{target_acc*100:.0f}% baseline: {data['n_features']} features "
            ...
            for group_name, data in sorted_groups:
                ...
                f.write(f"{group_name:20s} | Drop: {acc_drop:.4f} | Features removed: {data['n_features_removed']}\n")
    ...
    def create_ablation_visualizations(self, batch_name: str, results: Dict):
        ...
        fig.suptitle(f'Feature Ablation Analysis: {batch_name}', fontsize=16, fontweight='bold')
        ...
        ax2.errorbar(n_features, accuracies, yerr=stds, marker='o', linewidth=2, markersize=6)
        ax2.axhline(y=results['baseline']['RandomForest']['mean_accuracy'], 
                   color='red', linestyle='--', alpha=0.7, label='Baseline (all features)')
        ...
        bars = ax3.bar(range(len(group_names)), group_drops, color='lightblue', alpha=0.7)
        ...
        if n_feat_optimal:
            ax4.plot(n_feat_optimal, acc_optimal, 'o-', label='Optimal Features', linewidth=2, markersize=8)
        if n_feat_random:
            ax4.errorbar(n_feat_random, acc_random, yerr=acc_random_std, 
                        fmt='x--', label='Random Features', alpha=0.7, linewidth=2, markersize=8)
        ...
        plt.tight_layout()
        plt.savefig(batch_dir / f"{batch_name}_ablation_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_combined_ablation_summary(self, all_results: Dict):
        ...
        with open(summary_file, 'w') as f:
            ...
            for feature_name, data in sorted_cross_batch[:15]:
                f.write(f"{feature_name:25s} | Batches: {data['n_appearances']} | "
                    ...
                    for target_pct in [0.95, 0.90]:
                        ...
                        for key, data in cum_results.items():
                            if data['mean_accuracy'] >= target_acc:
                                f.write(f"  {target_pct*100:.0f}% baseline ({target_acc:.3f}): "
