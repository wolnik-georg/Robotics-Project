...
try:
    ...
    print("Warning: SHAP not available. Install with 'pip install shap' for advanced explanations.")
...
try:
    ...
    print("Warning: LIME not available. Install with 'pip install lime' for additional explanations.")
...
class SklearnFeatureSaliencyAnalyzer:
    ...
    def load_batch_features(self, batch_name: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        feature_columns = [col for col in df.columns if col not in ['simplified_label', 'original_label']]
    ...
    def _extract_feature_importance(self, model, model_name: str, X_train: np.ndarray, 
                                   y_train: np.ndarray, feature_names: List[str]) -> Dict:
        """Extract feature importance from different model types."""
        importance_dict = {}
        
        # Built-in feature importance (if available)
        if hasattr(model, 'feature_importances_'):
            importance_dict['builtin'] = dict(zip(feature_names, model.feature_importances_))
        ...
        try:
            ...
            importance_dict['permutation_std'] = dict(zip(feature_names, perm_importance.importances_std))
        except Exception as e:
            print(f"  Warning: Permutation importance failed for {model_name}: {e}")
        
        return importance_dict
    
    def compute_statistical_significance(self, batch_name: str, model_name: str = 'random_forest') -> Dict:
        """
        ...
        significant_features = {k: v for k, v in feature_significance.items() if v['is_significant']}
        print(f"\n✅ Found {len(significant_features)}/{len(feature_names)} statistically significant features")
        
        return feature_significance
    
    def compute_shap_values(self, batch_name: str, model_name: str = 'random_forest') -> Dict:
        """
    ...
    def compute_lime_explanations(self, batch_name: str, model_name: str = 'random_forest', 
    ...
    def analyze_batch_saliency(self, batch_name: str) -> Dict:
        ...
        try:
            ...
            try:
                significance = self.compute_statistical_significance(batch_name, 'random_forest')
            ...
            if SHAP_AVAILABLE:
                try:
                    ...
                    results['interpretability']['shap'] = shap_results.get('feature_importance', {})
                except Exception as e:
            ...
            if LIME_AVAILABLE:
                try:
                    lime_results = self.compute_lime_explanations(batch_name, 'random_forest')
                    results['interpretability']['lime'] = lime_results.get('avg_feature_importance', {})
                except Exception as e:
    ...
    def visualize_feature_importance(self, batch_name: str, save_dir: Path = None):
        ...
        fig.suptitle(f'Feature Importance Analysis: {batch_name}\n{results["config"]["description"]}', 
                    fontsize=16, fontweight='bold')
        
        # 1. Random Forest Feature Importance
        ax1 = axes[0, 0]
        if 'random_forest' in results['feature_importance'] and 'builtin' in results['feature_importance']['random_forest']:
            ...
            bars = ax1.barh(range(len(sorted_values)), sorted_values, color='skyblue', alpha=0.7)
        ...
        ax2 = axes[0, 1]
        if 'random_forest' in results['feature_importance'] and 'permutation' in results['feature_importance']['random_forest']:
            perm_importance = results['feature_importance']['random_forest']['permutation']
            ...
            bars = ax2.barh(range(len(sorted_values)), sorted_values, color='lightgreen', alpha=0.7)
        ...
        if 'statistical_significance' in results:
            ...
            p_values = [significance.get(name, {}).get('p_value', 1.0) for name in feature_names]
            score_drops = [significance.get(name, {}).get('score_drop', 0.0) for name in feature_names]
            
            # Only show significant features
            significant_mask = np.array(p_values) < 0.05
            if significant_mask.any():
                sig_names = [name for i, name in enumerate(feature_names) if significant_mask[i]]
                sig_drops = [score_drops[i] for i in range(len(score_drops)) if significant_mask[i]]
                ...
                ax3.barh(range(len(sorted_values)), sorted_values, color='coral', alpha=0.7)
                ax3.set_yticks(range(len(sorted_values)))
                ax3.set_yticklabels(sorted_names, fontsize=8)
                ax3.set_xlabel('Score Drop (Significance)')
                ax3.set_title(f'Statistically Significant Features ({len(sorted_names)})')
            else:
                ax3.text(0.5, 0.5, 'No statistically\nsignificant features\nfound', 
        ...
        try:
            ...
            if 'random_forest' in results['feature_importance'] and 'builtin' in results['feature_importance']['random_forest']:
                rf_scores = [results['feature_importance']['random_forest']['builtin'].get(name, 0) 
                           for name in feature_names]
                methods.append('RF Built-in')
                all_scores.append(rf_scores)
            
            if 'random_forest' in results['feature_importance'] and 'permutation' in results['feature_importance']['random_forest']:
                perm_scores = [results['feature_importance']['random_forest']['permutation'].get(name, 0) 
                             for name in feature_names]
            ...
            if len(methods) >= 2:
                ...
                ax4.text(0.5, 0.5, 'Need multiple methods\nfor comparison', 
                        ha='center', va='center', transform=ax4.transAxes)
                ax4.set_title('Method Comparison')
            
        except Exception as e:
            ax4.text(0.5, 0.5, f'Comparison failed:\n{str(e)[:50]}...', 
                    ha='center', va='center', transform=ax4.transAxes)
            ax4.set_title('Method Comparison')
        ...
        if save_dir:
            ...
            plt.savefig(save_dir / f"{batch_name}_feature_importance.png", dpi=300, bbox_inches='tight')
    ...
    def _save_saliency_results(self, batch_name: str, save_dir: Path):
        ...
        if 'random_forest' in results['feature_importance']:
            ...
            if 'builtin' in rf_importance:
                saliency_df['rf_builtin'] = [rf_importance['builtin'].get(name, 0) for name in feature_names]
            
            if 'permutation' in rf_importance:
                saliency_df['rf_permutation'] = [rf_importance['permutation'].get(name, 0) for name in feature_names]
                if 'permutation_std' in rf_importance:
                    saliency_df['rf_permutation_std'] = [rf_importance['permutation_std'].get(name, 0) for name in feature_names]
        
        # Add statistical significance
        if 'statistical_significance' in results:
            significance = results['statistical_significance']
            saliency_df['p_value'] = [significance.get(name, {}).get('p_value', 1.0) for name in feature_names]
            saliency_df['score_drop'] = [significance.get(name, {}).get('score_drop', 0.0) for name in feature_names]
            saliency_df['is_significant'] = [significance.get(name, {}).get('is_significant', False) for name in feature_names]
        ...
        if 'interpretability' in results:
            if 'shap' in results['interpretability']:
                ...
                saliency_df['shap_importance'] = [shap_importance.get(name, 0) for name in feature_names]
            
            if 'lime' in results['interpretability']:
                lime_importance = results['interpretability']['lime']
                saliency_df['lime_importance'] = [lime_importance.get(name, 0) for name in feature_names]
        
        # Sort by most important feature (RF built-in if available)
        sort_column = 'rf_builtin' if 'rf_builtin' in saliency_df.columns else saliency_df.columns[1]
        ...
        # Analysis summary
        if 'statistical_significance' in results:
            n_significant = saliency_df['is_significant'].sum() if 'is_significant' in saliency_df.columns else 0
    ...
    def _generate_combined_saliency_summary(self, all_results: Dict):
        ...
        summary_path = Path("batch_analysis_results/combined_feature_saliency_summary.txt")
        ...
        with open(summary_path, "w") as f:
            ...
            for batch_name, result in all_results.items():
                if 'feature_importance' in result and 'random_forest' in result['feature_importance']:
            ...
            consistent_features = {k: v for k, v in feature_votes.items() if len(v) >= 2}
            
            f.write("MOST CONSISTENT IMPORTANT FEATURES ACROSS BATCHES:\n")
            f.write("-" * 60 + "\n")
            
            for feature, batch_scores in sorted(consistent_features.items(), 
            ...
            for batch_name, result in all_results.items():
                ...
                if 'model_results' in result and 'random_forest' in result['model_results']:
                ...
                if 'statistical_significance' in result:
                    ...
                    n_significant = sum(1 for v in significance.values() if v.get('is_significant', False))
                    total_features = len(significance)
                    f.write(f"Statistically Significant Features: {n_significant}/{total_features} ({n_significant/total_features*100:.1f}%)\n")
