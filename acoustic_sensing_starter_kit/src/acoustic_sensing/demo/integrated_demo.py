...
class IntegratedAcousticSystem:
    ...
    def setup_training_pipeline(self):
        ...
        self.training_pipeline = ConfigurableTrainingPipeline(
            data_path=str(self.data_path), output_dir=str(self.output_dir)
        )
    ...
    def train_all_modes(self) -> dict:
        ...
        for mode in modes:
            ...
            self.logger.info(
                f"{mode} mode - Accuracy: {results['test_accuracy']:.3f}, Features: {len(results['feature_names'])}"
            )
    ...
    def setup_real_time_system(self, mode: str = "OPTIMAL"):
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                f"Model not found: {model_path}. Please train first."
            )
    ...
    def demonstrate_real_time_operation(self, duration_seconds: float = 30.0):
        ...
        try:
            while time.time() - start_time < duration_seconds:
                ...
                audio_chunk = (
                    np.random.randn(self.real_time_sensor.config.buffer_size) * 0.1
                )

                # Add some simulated contact events
                if np.random.random() < 0.1:  # 10% chance of contact simulation
                    audio_chunk += (
                        np.random.randn(len(audio_chunk)) * 0.5
                    )  # Add contact-like signal
                ...
                if success:
                    ...
                    if result and "error" not in result:
                        ...
                        if len(results_log) % 50 == 0:
                            ...
                            self.logger.info(
                                f"Processed {len(results_log)} samples, "
    ...
    def _run_timing_benchmark(self, num_samples: int = 1000) -> dict:
        ...
        for _ in range(num_samples):
            ...
            features = self.real_time_sensor.feature_extractor.extract_from_audio(
                audio_chunk
            )
    ...
    def _calculate_efficiency_score(
        self, processing_time_ms: float, accuracy: float, feature_count: int
    ) -> float:
        ...
        complexity_score = max(
            0, 1 - feature_count / 40.0
        )  # Normalize to 40 features max

        # Weighted combination
        efficiency_score = (
            0.4 * time_score + 0.4 * accuracy_score + 0.2 * complexity_score
        )
    ...
    def _create_training_comparison_plots(self, training_results: dict):
        ...
        feature_counts = [
            len(training_results[mode]["feature_names"]) for mode in modes
        ]
        ...
        if "OPTIMAL" in training_results:
            ...
            importance = training_results["OPTIMAL"].get(
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "training_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _create_benchmark_plots(self, benchmark_results: dict):
        ...
        # Efficiency scores
        efficiency_scores = [
            benchmark_results[mode]["efficiency_score"] for mode in modes
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "benchmark_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _analyze_demo_results(self, results_log: list):
        ...
        if len(df) > 0:
            ...
            plt.tight_layout()
            plt.savefig(
                self.output_dir / "demo_analysis.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
    ...
    def _print_workflow_summary(self, benchmark_results: dict):
        ...
        for mode in ["MINIMAL", "OPTIMAL", "RESEARCH"]:
            if mode in benchmark_results:
                ...
                print(
                    f"{mode:>8}: {result['accuracy']:.3f} accuracy, "
        ...
        best_accuracy = max(
            benchmark_results.keys(), key=lambda x: benchmark_results[x]["accuracy"]
        )
        best_speed = min(
            benchmark_results.keys(),
            key=lambda x: benchmark_results[x]["avg_processing_time_ms"],
        )
        best_efficiency = max(
            benchmark_results.keys(),
            key=lambda x: benchmark_results[x]["efficiency_score"],
        )

        print(
            f"• Best accuracy: {best_accuracy} mode ({benchmark_results[best_accuracy]['accuracy']:.3f})"
        )
        print(
            f"• Fastest processing: {best_speed} mode ({benchmark_results[best_speed]['avg_processing_time_ms']:.2f}ms)"
        )
        print(
            f"• Best efficiency: {best_efficiency} mode (score: {benchmark_results[best_efficiency]['efficiency_score']:.3f})"
        )
        ...
        print(
            f"For research applications: Use {best_accuracy} mode for maximum accuracy"
        )
