...
class FrequencyBandAblationAnalyzer:
    ...
    def __init__(
        ...
        output_dir: str = "frequency_band_analysis_results",
    ):
    ...
    def apply_frequency_filter(
        self, audio: np.ndarray, freq_range: Tuple[float, float]
    ) -> np.ndarray:
    ...
    def extract_features_from_filtered_audio(
        self, audio_data: List[np.ndarray], freq_range: Tuple[float, float]
    ) -> np.ndarray:
        ...
        for i, audio in enumerate(audio_data):
            try:
                ...
                features = feature_extractor.extract_features(
                    filtered_audio, method="comprehensive"
                )
                ...
                if filtered_features:
                    ...
                    dummy_features = np.zeros(
                        38
                    )  # Standard number of acoustic features
    ...
    def analyze_batch_frequency_bands(
        ...
        for band_name, freq_range in self.frequency_bands.items():
            ...
            try:
                ...
                X_band = self.extract_features_from_filtered_audio(
                    audio_data, freq_range
                )
    ...
    def perform_statistical_analysis(self, frequency_results: Dict) -> Dict:
        ...
        stats_results = {"anova_test": {}, "pairwise_tests": {}, "band_rankings": {}}
        ...
        band_means = {band: np.mean(scores) for band, scores in band_accuracies.items()}
        ...
        stats_results["band_rankings"] = {
            ...
            "proposed_rank": next(
                (i for i, (band, _) in enumerate(ranked_bands) if band == "proposed"),
                -1,
            )
            + 1,
        }
    ...
    def identify_best_bands(self, frequency_results: Dict) -> Dict:
        ...
        for clf_name in self.classifiers.keys():
            clf_performances = [
                p for p in all_performances if p["classifier"] == clf_name
            ]
            if clf_performances:
        ...
        if proposed_performances:
            best_results["proposed_band_performance"] = {
                ...
                "mean_across_classifiers": np.mean(
                    [p["accuracy"] for p in proposed_performances]
                ),
                "std_across_classifiers": np.std(
                    [p["accuracy"] for p in proposed_performances]
                ),
            }
    ...
    def create_visualizations(self, batch_name: str, results: Dict):
        ...
        self._plot_band_performance_comparison(
            batch_name, freq_results, batch_output_dir
        )
        ...
        if stats_results:
            self._plot_statistical_significance(
                batch_name, stats_results, batch_output_dir
            )

        # 3. Frequency Range Performance Surface
        self._plot_frequency_performance_surface(
            batch_name, freq_results, batch_output_dir
        )
    ...
    def _plot_band_performance_comparison(
        self, batch_name: str, freq_results: Dict, output_dir: Path
    ):
        ...
        sorted_data = sorted(
            zip(bands, accuracies, std_errs, freq_ranges, freq_centers),
            key=lambda x: x[1],
            reverse=True,
        )
        bands_sorted, acc_sorted, err_sorted, range_sorted, center_sorted = zip(
            *sorted_data
        )
        ...
        ax1.set_xlabel(
            "Frequency Bands (Sorted by Performance)", fontsize=12, fontweight="bold"
        )
        ax1.set_ylabel("Classification Accuracy", fontsize=12, fontweight="bold")
        ax1.set_title(
            f"🎯 Frequency Band Performance Ranking\n{batch_name}",
            fontsize=14,
            fontweight="bold",
        )
        ax1.set_xticks(range(len(bands_sorted)))
        ax1.set_xticklabels(
            [f"{i+1}. {band}" for i, band in enumerate(bands_sorted)],
            rotation=45,
            ha="right",
            fontsize=9,
        )
        ...
        # Create table data
        table_data = []
        for i, (band, acc, err) in enumerate(
            zip(bands_sorted[:8], acc_sorted[:8], err_sorted[:8])
        ):
        ...
        ax3.set_title(
            "📊 Top Performance Summary", fontsize=12, fontweight="bold", pad=20
        )

        plt.tight_layout()
        plt.savefig(
            output_dir / f"{batch_name}_frequency_band_performance.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _plot_proposed_band_analysis(
        self, batch_name: str, freq_results: Dict, output_dir: Path
    ):
        ...
        bars = ax1.bar(
            top_names, top_accs, yerr=top_errs, color=colors, alpha=0.8, capsize=8
        )
        ...
        ax1.set_ylabel("Classification Accuracy", fontsize=12, fontweight="bold")
        ax1.set_title(
            "🚨 Proposed Band vs Top Performers", fontsize=14, fontweight="bold"
        )
        ax1.grid(True, alpha=0.3, axis="y")
        ax1.set_ylim(0, 1.05)
        ...
        ax3.set_title(
            "🔍 Proposed Band Components Analysis", fontsize=12, fontweight="bold"
        )
...
✅ VALIDATION STATUS: HYPOTHESIS REJECTED
   ...
   discriminative information is NOT supported.
        ...
        plt.suptitle(
            f"🎯 Frequency Band Hypothesis Validation: {batch_name}",
            fontsize=16,
            fontweight="bold",
            y=0.98,
        )
        plt.tight_layout()
        plt.savefig(
            output_dir / f"{batch_name}_proposed_band_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()

    def _plot_statistical_significance(
        self, batch_name: str, stats_results: Dict, output_dir: Path
    ):
        ...
        try:
            ...
            proposed_better = [
                pairwise_tests[band]["proposed_better"] for band in bands
            ]
            ...
            ax1.axhline(
                y=0.05, color="red", linestyle="--", alpha=0.8, label="p=0.05 threshold"
            )
            ax1.set_xlabel(
                "Frequency Band (vs Proposed)", fontsize=12, fontweight="bold"
            )
            ax1.set_ylabel("P-value (log scale)", fontsize=12, fontweight="bold")
            ax1.set_title(
                f"🔬 Statistical Significance vs Proposed Band\n{batch_name}",
                fontsize=14,
                fontweight="bold",
            )
            ...
            for band in bands:
                ...
                effect_magnitudes.append(
                    1 if proposed_better[bands.index(band)] else -1
                )

            effect_colors = [
                "blue" if better else "orange" for better in proposed_better
            ]
            bars2 = ax2.bar(
                range(len(bands)), effect_magnitudes, color=effect_colors, alpha=0.7
            )

            ax2.set_xlabel("Frequency Band", fontsize=12, fontweight="bold")
            ax2.set_ylabel("Effect Direction", fontsize=12, fontweight="bold")
            ax2.set_title(
                "📊 Proposed Band vs Other Bands\nPerformance Direction",
                fontsize=12,
                fontweight="bold",
            )
            ...
            plt.tight_layout()
            plt.savefig(
                output_dir / f"{batch_name}_statistical_significance.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _create_text_statistical_summary(
        self, batch_name: str, stats_results: Dict, output_dir: Path
    ):
        ...
        with open(summary_path, "w") as f:
            ...
            for band, test_result in pairwise_tests.items():
                ...
                status = (
                    ...
                    if is_sig and is_better
                    else (
                        "SIGNIFICANTLY WORSE"
                        if is_sig and not is_better
    ...
    def _plot_frequency_performance_surface(
        self, batch_name: str, freq_results: Dict, output_dir: Path
    ):
        ...
        try:
            ...
            for band_name, band_data in freq_results.items():
                ...
                rf_results = band_data.get("classifier_results", {}).get(
                    "random_forest"
                )
                if rf_results and "mean_accuracy" in rf_results:
                    freq_range = band_data["freq_range"]
            ...
            for i, (fmin, fmax, perf, name) in enumerate(
                zip(freq_mins, freq_maxs, performances, band_names)
            ):
                ...
                ax.text(
                    ...
                    va="center",
                    fontweight="bold",
                    color="white" if name == "proposed" else "black",
                    fontsize=10,
                )
            ...
            ax.set_title(
                f"🎯 Frequency Band Performance Landscape\n{batch_name}",
                fontsize=14,
                fontweight="bold",
            )
            ax.set_xscale("log")
            ...
            legend_elements = [
                ...
                Patch(
                    facecolor="orange", alpha=0.8, label="Medium Performers (80-90%)"
                ),
            ...
            plt.tight_layout()
            plt.savefig(
                output_dir / f"{batch_name}_frequency_landscape.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        try:
            ...
            try:
                ...
                sns.heatmap(
                    performance_matrix,
                    xticklabels=[
                        name.replace("_", " ").title() for name in classifier_names
                    ],
            ...
            ax.set_title(
                f"🎯 Classification Performance Heatmap\n{batch_name}",
                fontsize=14,
                fontweight="bold",
            )
            ...
            plt.tight_layout()
            plt.savefig(
                output_dir / f"{batch_name}_classifier_comparison.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def analyze_all_batches(
        self, batch_names: Optional[List[str]] = None, save_results: bool = True
    ) -> Dict:
        ...
        for batch_name in batch_names:
            try:
                batch_results = self.analyze_batch_frequency_bands(
                    batch_name, save_results
                )
    ...
    def create_cross_batch_analysis(self, all_results: Dict):
        ...
        for batch_name, batch_results in all_results.items():
            ...
            for band_name, band_data in freq_results.items():
                ...
                for clf_name, clf_results in band_data.get(
                    "classifier_results", {}
                ).items():
        ...
        df_combined.to_csv(
            self.output_dir / "combined_frequency_band_analysis.csv", index=False
        )
    ...
    def _create_cross_batch_visualizations(self, df_combined: pd.DataFrame):
        ...
        if not proposed_data.empty:
            ...
            axes[0, 0].set_title(
                "Proposed Band (200-2000Hz) Performance Across Batches"
            )
        ...
        best_bands = df_combined.loc[
            df_combined.groupby(["batch", "classifier"])["mean_accuracy"].idxmax()
        ]

        for batch in df_combined["batch"].unique():
            batch_best = best_bands[best_bands["batch"] == batch]
            freq_centers = [
                (row["freq_min_hz"] + row["freq_max_hz"]) / 2
                for _, row in batch_best.iterrows()
            ]
        ...
        band_rankings = {}
        for batch in df_combined["batch"].unique():
            batch_data = df_combined[
                (df_combined["batch"] == batch)
                & (df_combined["classifier"] == "random_forest")
            ]
            if not batch_data.empty:
                sorted_bands = batch_data.sort_values("mean_accuracy", ascending=False)[
                    "frequency_band"
                ].tolist()
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "cross_batch_frequency_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()

    def _generate_frequency_analysis_report(
        self, df_combined: pd.DataFrame, all_results: Dict
    ):
        ...
        with open(report_path, "w") as f:
            ...
            if not proposed_data.empty:
                ...
                f.write(
                    f"- Mean accuracy across batches: {proposed_mean:.4f} ± {proposed_std:.4f}\n"
                )
            ...
            if total_comparisons > 0:
                ...
                f.write(
                    f"**Statistical Significance:** {significant_count}/{total_comparisons} "
                )
                f.write(
                    f"({sig_percentage:.1f}%) comparisons showed statistically significant differences.\n\n"
                )
            ...
            band_performance = (
                df_combined[df_combined["classifier"] == "random_forest"]
                .groupby("frequency_band")["mean_accuracy"]
                .agg(["mean", "std", "count"])
            )
            ...
            for rank, (band, row) in enumerate(band_performance.iterrows(), 1):
                f.write(
                    f"| {rank} | {band} | {row['mean']:.4f} | {row['std']:.4f} | {int(row['count'])} |\n"
                )
            ...
            if proposed_rank:
                if proposed_rank <= 3:
                    f.write(
                        f"✅ **VALIDATED**: The proposed 200-2000Hz frequency band ranks #{proposed_rank} "
                    )
                    f.write(
                        "among all tested bands, supporting the hypothesis that this range contains "
                    )
                    f.write(
                        "the most discriminative information for geometric contact classification.\n\n"
                    )
                else:
                    f.write(
                        f"⚠️ **MIXED RESULTS**: The proposed 200-2000Hz frequency band ranks #{proposed_rank}. "
                    )
                    f.write(
                        "Consider investigating other frequency ranges that performed better.\n\n"
                    )
    ...
    try:
        ...
        if results:
            ...
            if proposed_performances:
                ...
                print(
                    f"   Mean accuracy across {len(proposed_performances)} batches: {overall_mean:.4f} ± {overall_std:.4f}"
                )
