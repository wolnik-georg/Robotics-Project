...
class ImpulseResponseExperiment(BaseExperiment):
    ...
    def run(self, shared_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Save per-batch results and create per-batch plots
        for batch_name in impulse_responses.keys():
            ...
            self._create_batch_impulse_response_plots(batch_impulse_data, batch_name)
    ...
    def _create_batch_impulse_response_plots(
        ...
        if batch_frequency_analysis:
            freqs = batch_frequency_analysis.get("frequencies", [])
            magnitudes = batch_frequency_analysis.get("magnitude_responses", [])
