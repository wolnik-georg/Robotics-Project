...
class ConfigurableTrainingPipeline:
    ...
    def set_feature_mode(self, mode: str):
        ...
        print(f"   Expected: {self.feature_configs[mode].config['expected_performance']}")
        
    def load_and_filter_features(self, batch_name: str, mode: str = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(selected_indices) != config.n_features:
            missing_features = set(config.features) - set([all_feature_names[i] for i in selected_indices])
        ...
        print(f"  Loaded {batch_name}: {X_filtered.shape[0]} samples, {X_filtered.shape[1]} features")
    ...
    def train_single_batch(self, batch_name: str, mode: str = None) -> Dict:
        ...
        for model_name, model in models.items():
            ...
            print(f"  {model_name}: Test={test_acc:.3f}, CV={cv_scores.mean():.3f}±{cv_scores.std():.3f}")
    ...
    def create_comparison_plot(self, comparison_results: Dict, batch_name: str):
        ...
        fig.suptitle(f'Feature Mode Comparison: {batch_name}', fontsize=16, fontweight='bold')
        ...
        bars1 = ax1.bar(modes, rf_accuracies, color=['lightcoral', 'lightblue', 'lightgreen'])
        ...
        bars2 = ax2.bar(modes, n_features, color=['lightcoral', 'lightblue', 'lightgreen'])
        ...
        for mode in modes:
            if mode in comparison_results:
                time_ms = comparison_results[mode]['models']['RandomForest']['training_time_ms']
                training_times.append(time_ms)
        
        bars3 = ax3.bar(modes, training_times, color=['lightcoral', 'lightblue', 'lightgreen'])
        ax3.set_ylabel('Training Time (ms)')
        ax3.set_title('Training Time by Feature Set')
        
        # 4. Accuracy vs Features scatter
        ax4.scatter(n_features, rf_accuracies, s=100, c=['red', 'blue', 'green'], alpha=0.7)
        for i, mode in enumerate(modes):
            ax4.annotate(mode, (n_features[i], rf_accuracies[i]), 
...
def main():
    ...
    parser.add_argument('--mode', choices=['MINIMAL', 'OPTIMAL', 'RESEARCH'], 
                       default='OPTIMAL', help='Feature set mode')
    parser.add_argument('--batch', type=str, help='Specific batch to train')
    parser.add_argument('--compare-all', action='store_true', 
                       help='Compare all feature modes')
    parser.add_argument('--compare-batch', type=str, 
                       help='Compare modes for specific batch')
