    ...
    def get_feature_indices(self, all_feature_names: List[str]) -> List[int]:
        ...
        for feature in self.features:
            if feature in all_feature_names:
                ...
                print(f"Warning: Feature '{feature}' not found in provided feature list")
        return indices
        
    def filter_features(self, feature_array: np.ndarray, 
        ...
        return feature_array[:, indices] if len(feature_array.shape) > 1 else feature_array[indices]
...
def extract_optimized_features(audio_signal: np.ndarray, 
