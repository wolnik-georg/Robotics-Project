...
Peaklim::Peaklim (void) :
    ...
    _gmin (1)
{
    for (int i = 0; i < MAXCHAN; i++) _dbuff [i] = 0;
}
...
void Peaklim::set_inpgain (float v)
{
    _g1 = powf (10.0f, 0.05f * v);
}


void Peaklim::set_threshold (float v)
{
    _gt = powf (10.0f, -0.05f * v);
}


void Peaklim::set_release (float v)
{
    if (v > 1.0f)  v = 1.0f;
    if (v < 1e-3f) v = 1e-3f;
    _w3 = 1.0f / (v * _fsamp);
...
void Peaklim::init (float fsamp, int nchan)
{
    int i, k1, k2;
    ...
    for (i = 0; i < _nchan; i++) _zlf [i] = 0.0f;
    _z1 = 1.0f;
    _z2 = 1.0f;
    _z3 = 1.0f;
    _gt = 1.0f;
...
void Peaklim::process (int nframes, float *inp[], float *out[])
{
    int     i, j, k, n, ri, wi;
