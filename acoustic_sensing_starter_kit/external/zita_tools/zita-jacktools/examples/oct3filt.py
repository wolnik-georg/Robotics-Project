...
# Measure frequency response of third octave band filters in JackIECfilt.
#

# We measure four filters at a time.
...
F = JackIECfilt (1, 4, "JackIECfilt")
F.connect_input (0, "JackSignal:out")
for i in range (4):
    F.connect_output (i, "JackSignal:in")
...
# Generate data.
#
Aout = np.zeros ([10,], dtype = np.float32)
Aout [0] = 1.0
...
# Measure and plot all bands, four at a time.
#
for i in range (0, 8):
    print ("measuring bands", end = "")
    for j in range (0, 4):
        k = i + 8 * j
        if k <= 30:
            F.set_filter (0, j, 3, k)
            print (" %2d" % (k,), end = "") 
        else:
            F.set_filter (0, j, 0, 0)
    print ()
    J.process()
    J.wait()
