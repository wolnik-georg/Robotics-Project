...
\begin{frame}{Motivation}
    \begin{columns}[T]
        % Left column: The Core Idea
        \begin{column}{0.48\textwidth}
            \begin{block}{The Core Idea}
                \textbf{Every sensor creates representations}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Cameras $\rightarrow$ Images of the environment
                    \item LiDAR $\rightarrow$ 3D point clouds
                    \item Force sensors $\rightarrow$ Contact force maps
                \end{itemize}
            
                
                \textbf{\color{highlightblue}Why not acoustic sensors?}\\[0.1cm]
                {\small If a robot finger has a microphone, it should also create a representation of what it touches!}
            \end{block}
            
            \begin{alertblock}{The Vision}
                \centering
                \textbf{Reconstruct what the finger touches}\\[0.1cm]
                {\small Not replacing cameras --- but sensing what cameras \textbf{cannot} see}
            \end{alertblock}
        \end{column}
        
        % Right column: Why Geometry + Goal
        \begin{column}{0.48\textwidth}
            \begin{block}{Why Start With Geometry?}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Geometric shapes are \textbf{building blocks}
                    \item Screws, buttons, edges, holes...
                    \item Complex objects = combinations of primitives
                    \item \textbf{If we can reconstruct geometry,}\\we can reconstruct anything
                \end{itemize}
            \end{block}
            
            \begin{block}{Project Goal}
                \begin{center}
                    \textit{Can acoustic sensing create visual representations of touched surfaces?}
                \end{center}
                {\small Binary contact classification $\rightarrow$ Surface reconstruction}
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            \node[fill=highlightblue!10, rounded corners, inner sep=6pt, text width=0.9\textwidth] {
                \footnotesize\textbf{\color{highlightblue}Why Use a Robot? --- It's Part of the System!}
                \begin{itemize}\setlength{\itemsep}{1pt}\setlength{\parskip}{0pt}
                    \item[\faCheck] \textbf{Mechanically coupled:} Sound travels through arm joints and links to the microphone
                    \item[\faCheck] \textbf{Real-world relevance:} In actual deployment, the sensor \textit{will} be on a robot
                    \item[\faCheck] \textbf{Controlled experiments:} Systematic, repeatable data collection across configurations
                \end{itemize}
            };
        \end{tikzpicture}
    \end{center}
...
% SLIDE 3b: THE HIDDEN CHALLENGE - Robot Arm Affects Signal
\begin{frame}{The Hidden Challenge: Robot Arm Affects the Signal}
    
    \begin{columns}[T]
        \begin{column}{0.58\textwidth}
            \begin{alertblock}{\centering \faExclamationTriangle{} The Problem}
                \centering
                \large
                \textbf{The robot arm is part of the acoustic system}
                
                \footnotesize
                Sound waves travel through:\\
                Surface $\rightarrow$ Probe $\rightarrow$ \textbf{Robot Arm} $\rightarrow$ Microphone
            \end{alertblock}
            ...
            \begin{block}{What This Means}
                \footnotesize
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Different arm positions} = Different acoustic responses
                    \item Joint angles change wave propagation paths
                    \item Same contact event sounds \textbf{different} at different positions
                \end{itemize}
            \end{block}
            ...
            \begin{block}{The Key Insight}
                \centering
                \footnotesize
                \textbf{Signal = Contact Information $\otimes$ Arm Configuration}\\[0.0cm]
                The signal contains \textbf{both} --- they are entangled!\\[0.1cm]
                {\scriptsize (Also observed in VibeCheck [Zoller et al., 2024])}
            \end{block}
        \end{column}
        
        \begin{column}{0.42\textwidth}
            \begin{center}
                \textbf{\color{highlightblue}Why Multiple Workspaces?}
                
                
                \begin{tikzpicture}[scale=0.8, transform shape]
                    % Workspace 1
                    \node[fill=highlightblue!20, rounded corners, minimum width=4cm, minimum height=0.8cm] at (0,3) {\textbf{WS1: Position A}};
                    \node[font=\scriptsize, text=gray] at (0,2.4) {Arm config $\theta_1$};
                    
                    % Workspace 2
                    \node[fill=highlightblue!20, rounded corners, minimum width=4cm, minimum height=0.8cm] at (0,1.5) {\textbf{WS2: Position B}};
                    \node[font=\scriptsize, text=gray] at (0,0.9) {Arm config $\theta_2$};
                    
                    % Workspace 3
                    \node[fill=highlightblue!20, rounded corners, minimum width=4cm, minimum height=0.8cm] at (0,0) {\textbf{WS3: Position C}};
                    \node[font=\scriptsize, text=gray] at (0,-0.6) {Arm config $\theta_3$};
                \end{tikzpicture}

                
                \begin{tikzpicture}
                    \node[fill=orange!15, rounded corners, inner sep=8pt, text width=4.5cm, align=center] {
                        \footnotesize
                        \textbf{Question:}\\
                        Will a classifier trained at one position work at another?
                    };
                \end{tikzpicture}
            \end{center}
        \end{column}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{\centering Target}
                ...
                \textbf{on data from the same distribution}
                \vspace{0.2cm}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.25\textwidth}
            \begin{alertblock}{Why Hand-Crafted?}
                \small
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Hand-crafted: \textcolor{successgreen}{\textbf{70\%}}
...
\begin{frame}{Result: \textcolor{successgreen}{Excellent Classification on Data From Same Distribution}}
...
\begin{frame}{Reconstruction: Excellent Performance on Test Data}
    ...
    \begin{columns}[T]
        \begin{column}{0.35\textwidth}
            \begin{center}
                \textbf{\color{highlightblue}Object View}
                
                \vspace{0.2cm}
                
                \includegraphics[width=\textwidth, height=0.70\textheight, keepaspectratio]{image1.jpeg}
            \end{center}
        \end{column}
        
        \begin{column}{0.63\textwidth}
            \begin{center}
                \textbf{\color{highlightblue}Ground Truth vs Prediction}
                
                \vspace{0.2cm}
                
                \includegraphics[width=\textwidth, height=0.70\textheight, keepaspectratio]{balanced_workspace_2_squares_cutout_comparison.png}
            \end{center}
        \end{column}
    \end{columns}
...
% SLIDE 8b: RECONSTRUCTION EXAMPLE - Full Contact
\begin{frame}{Reconstruction: Full Contact Surface}
    \begin{columns}[T]
                \begin{column}{0.35\textwidth}
            \begin{center}
                \textbf{\color{highlightblue}Object View}
                ...
                \includegraphics[width=\textwidth, height=0.65\textheight, keepaspectratio]{image2.jpeg}
            \end{center}
        \end{column}
        ...
        \begin{column}{0.63\textwidth}
            \begin{center}
                ...
                \includegraphics[width=\textwidth, height=0.65\textheight, keepaspectratio]{balanced_workspace_2_pure_contact_comparison.png}
            \end{center}
        \end{column}
    \end{columns}
...
% SLIDE 8c: RECONSTRUCTION EXAMPLE - No Contact
\begin{frame}{Reconstruction: No-Contact Surface}
    ...
    \begin{columns}[T]
        \begin{column}{0.9\textwidth}
            \begin{center}
                ...
                \includegraphics[width=\textwidth, height=0.65\textheight, keepaspectratio]{balanced_workspace_2_pure_no_contact_comparison.png}
            \end{center}
        \end{column}
    \end{columns}
...
\begin{frame}{Now: Testing Generalization}
    ...
    \begin{block}{\centering \textcolor{successgreen}{\faCheckCircle} Proof of Concept Achieved!}
        \centering
        \large
        \textbf{Acoustic contact classification works on data from the same distribution}\\[0.2cm]
        \footnotesize
        ...
        \begin{column}{0.52\textwidth}
            \begin{block}{\centering Performance Results}
                ...
                \begin{tabular}{lc}
                    ...
                    \midrule
                    \textbf{Validation (WS2)} & \textcolor{orange}{\textbf{65-70\%}} \\
                \end{tabular}
            \end{block}
            ...
            \begin{alertblock}{\centering Configuration Entanglement}
                \centering
                {\Large \textcolor{orange}{\textbf{65-70\%}}}\\[0.1cm]
                \footnotesize
...
% SLIDE 12b: VISUAL SUMMARY - Reconstruction Comparison
\begin{frame}{Reconstruction on New Object + Workspace Fails}
    
    \begin{columns}[T]
        \begin{column}{0.35\textwidth}
            \vspace{0.9cm}
            \begin{center}
                \includegraphics[width=\textwidth, height=0.55\textheight, keepaspectratio]{image3.jpeg}
            \end{center}
        \end{column}
        
        \begin{column}{0.63\textwidth}
            \begin{center}
                \textbf{\color{failurered}New Object + WS}
                
                \vspace{0.1cm}
                
                {\scriptsize Train: A-C $\rightarrow$ Test: D on WS4}
                
                \vspace{0.2cm}
                
                \includegraphics[width=\textwidth, height=0.55\textheight, keepaspectratio]{balanced_holdout_oversample_comparison.png}
                
                \vspace{0.1cm}
                
                {\Large \textcolor{failurered}{\textbf{50\%}}}
            \end{center}
        \end{column}
    \end{columns}
    ...
    \begin{center}
        \begin{tikzpicture}
            \node[fill=highlightblue!10, rounded corners, inner sep=10pt] {
                \footnotesize \textbf{Key Insight:} Performance degrades as we move away from training distribution
            };
        \end{tikzpicture}
    \end{center}
...
\begin{frame}{Conclusions}
    ...
    \begin{block}{\textcolor{successgreen}{\faCheckCircle} Main Achievement: Proof of Concept}
        ...
        \footnotesize
        (on data from the same distribution: $\sim$100\% test accuracy, excellent reconstruction)
    \end{block}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{What Works}
                ...
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item Hand-crafted features outperform\\spectrograms (70\% vs 59\%)
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Configuration Generalization Test}
                ...
                \begin{itemize}\setlength{\itemsep}{2pt}
                    ...
                    \item Result: \textcolor{orange}{\textbf{65-70\%}} (partial)
                \end{itemize}
            \end{block}
        ...
        \begin{column}{0.42\textwidth}
            ...
            \begin{block}{Implications}
                ...
                \begin{itemize}\setlength{\itemsep}{1pt}
                    ...
                    \item Configuration: 65-70\% (partial)
