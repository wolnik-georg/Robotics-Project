"""
Pattern A Reconstruction with CONSISTENT confidence filtering.
...
Pattern A: WS2+WS3 train → WS1 validation
"""
...
# Use the original Pattern A model (trained with confidence filtering)
MODEL_PATH = "training_for_reconstruction_v1/discriminationanalysis/trained_models/model_rank1_random_forest.pkl"
...
VALIDATION_DATASETS = [
    ("VAL_WS1_squares_cutout", "data/balanced_workspace_1_squares_cutout_oversample"),
    ("VAL_WS1_pure_contact", "data/balanced_workspace_1_pure_contact_oversample"),
    ("VAL_WS1_pure_no_contact", "data/balanced_workspace_1_pure_no_contact_oversample"),
]

fe = GeometricFeatureExtractor(
    use_workspace_invariant=True, use_impulse_features=True, sr=48000
)
output_base = Path("pattern_a_consistent_reconstruction")
output_base.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("PATTERN A RECONSTRUCTION - CONSISTENT CONFIDENCE FILTERING")
print(
...
for ds_name, ds_path in VALIDATION_DATASETS:
    if not Path(ds_path).exists():
        print(f"  ⚠️ {ds_name}: Dataset not found")
        continue
...
# Summary
print("\n" + "=" * 80)
print("📋 PATTERN A SUMMARY (CONSISTENT WITH TRAINING)")
print("=" * 80)

test_results = [r for r in all_results if r["type"] == "TEST"]
val_results = [r for r in all_results if r["type"] == "VALIDATION"]
...
if val_results:
    val_overall_avg = np.mean([r["overall_acc"] for r in val_results])
    val_highconf_avg = np.mean(
        [r["high_conf_acc"] for r in val_results if r["high_conf_acc"] > 0]
    )
    val_coverage = np.mean([r["pct_high_conf"] for r in val_results])

    print(f"\nVALIDATION (WS1):")
    print(f"  Overall accuracy (all samples):  {val_overall_avg:.1f}%")
    print(f"  High-conf accuracy (≥90% conf):  {val_highconf_avg:.1f}%")
    print(f"  High-conf coverage:              {val_coverage:.1f}%")

print(f"\nGENERALIZATION GAP (using high-conf accuracy for consistency):")
print(f"  TEST high-conf:       {test_highconf_avg:.1f}%")
print(f"  VALIDATION high-conf: {val_highconf_avg:.1f}%")
print(f"  Gap:                  {test_highconf_avg - val_highconf_avg:.1f}%")
