...
This work investigates acoustic sensing for contact detection and geometric reconstruction on rigid robotic manipulators. While soft robots leverage acoustic signals for proprioception, rigid manipulators have received limited attention despite advantages over vision-based methods in cluttered environments. We address four research questions: (1) Can acoustic sensing achieve above-random-chance contact detection? (2) Do learned models generalize across robot configurations? (3) Does including edge cases improve performance? (4) Can models generalize to novel object geometries? Using a Franka Panda manipulator with 4 contact objects across 4 workspaces, we collect perfectly balanced datasets across three contact states (contact, no-contact, edge at 33/33/33\% splits) and systematically evaluate through 5-fold cross-validation, 3 workspace rotations, and holdout testing. Results demonstrate proof-of-concept viability: 69.9\% cross-validation accuracy (2.10× over random baseline). However, cross-workspace generalization fails catastrophically, with validation accuracy ranging 23.3--55.7\% (average 34.5\%, barely above random chance). Critically, 3-class classification outperforms binary when properly normalized (1.04× vs 0.90× over random)---binary classification performs worse than guessing, proving edge samples contain essential discriminative information. Object generalization to novel geometry (Object D) reveals mixed results reproduced across 5 independent random seeds (std=0.0\%): most classifiers fail at 41.7\% (3-class) and exactly 50\% (binary, pure random chance), but heavily-regularized GPU-MLP (dropout 0.3, weight decay 0.01) achieves 75.0\% validation consistently across all seeds, demonstrating that proper regularization enables geometry-invariant learning. Physics-based analysis reveals acoustic signatures are fundamentally workspace-specific and object-specific. These findings establish that acoustic contact sensing achieves proof-of-concept within-workspace performance but requires both workspace-specific training and heavy regularization for deployment, with binary classification guaranteeing failure on novel objects (50\% = random chance).
...
\caption{Position generalization: 3-class reconstruction on Workspace 2 validation data (held out in Rotation 1). \textbf{Top row}: Ground truth contact patterns for objects A (cutout - balanced across all 3 classes), B (empty workspace - pure no-contact), and C (full contact - contact and edge only). \textbf{Bottom row}: Model predictions from acoustic features alone. Trained on WS1+WS3, the model achieves 55.7\% accuracy---moderate generalization that significantly exceeds random chance (33.3\%) but demonstrates substantial performance degradation from the 69.1\% cross-validation accuracy, revealing workspace-dependent acoustic signatures. Color indicates 3-class contact state (contact=green, no-contact=red, edge=orange).}
...
\begin{table}[t]
\centering
\caption{Object Generalization Results (3-Class): Training on Objects A, B, C (WS1+2+3) → Validation on Object D (WS4). Results reproduced across 5 seeds with std=0.0\%.}
...
\textbf{GPU-MLP (Medium-HighReg)} & \textbf{57.2\% ± 0.9\%} & \textbf{75.0\%} & \textbf{-17.8\%} \\
...
\begin{table}[t]
\centering
\caption{Object Generalization Results (Binary): Training on Objects A, B, C (WS1+2+3) → Validation on Object D (WS4). All classifiers collapse to exactly 50\% (std=0.0\%).}
...
GPU-MLP (Medium-HighReg) & 78.7\% ± 0.7\% & 49.9\% & 28.8\% \\
