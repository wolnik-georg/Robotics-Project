...
\begin{itemize}
    ...
    \item Contact position: 4 distinct clusters (tip/middle/base/no contact)
    \item Edge detection: 3 well-separated groups
    \item Paper clip detection: 2 classes with some overlap
\end{itemize}
...
\begin{itemize}
    ...
    \item Nonlinear methods show tighter clusters
    \item Consistent performance across visualization methods
\end{itemize}
...
\begin{frame}{Transfer Function: System Response Analysis}
\footnotesize
\textbf{Complete System Response Analysis:}

\vspace{0.5em}
\textbf{What It Shows:}
\begin{itemize}
    \item Input sweep spectrum (top-left)
    \item Response spectrum (top-right) 
    \item System transfer function (bottom-left)
    \item Corresponding time-domain waveform (bottom-right)
\end{itemize}

\vspace{0.5em}
\textbf{Key Insights:}
\begin{itemize}
    \item Transfer function reveals frequency-dependent system behavior
    \item Resonances and anti-resonances characterize contact geometry
    \item Damping properties affect peak shapes and widths
\end{itemize}

\includegraphics[width=\textwidth, height=0.6\textheight]{soft_finger_batch_2_impulse_response_analysis.png}
\end{frame}
